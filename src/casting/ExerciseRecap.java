...
public class ExerciseRecap {
    public static void main(String[] args) {
       ...
       /*
        ...
        Scanner scan = new Scanner(System.in); // import scanner first. create the object


        System.out.println("Please enter five of your grades"); // here you are asking
        //String grade1 = scan.next(), g2 = scan.next(), g3 = scan.next(),
        //g4 = scan.next(),g5 = scan.next(); // here you are using scanner for user input

        // double g1D = Double.parseDouble(grade1);  here you can declare and assign like this or use
        //double g2D = Double.parseDouble(grade1);     multiple declarations and assignments as done in line 27-29
        // double g3D = Double.parseDouble(grade1);
        //double g4D = Double.parseDouble(grade1);
        // double g5D = Double.parseDouble(grade1);

        //double g1D = Double.parseDouble(grade1), g2D = Double.parseDouble(grade1),
        // g3D = Double.parseDouble(grade1),g4D = Double.parseDouble(grade1),
        //g5D = Double.parseDouble(grade1);  here this way isn't necessary because you only used data once. can
        // simply put the scanner object instead of storing it like you did line 18-19
        //double g1D = Double.parseDouble(scan.next()), g2D = Double.parseDouble(scan.next()),
        //g3D = Double.parseDouble(scan.next()),g4D = Double.parseDouble(scan.next()),
        // g5D = Double.parseDouble(scan.next()); //Here instead of storing the user input as a double in g1D you can simply just print it

        System.out.println((int) (Double.parseDouble(scan.next()) +
                Double.parseDouble(scan.next())
                + Double.parseDouble(scan.next())
                + Double.parseDouble(scan.next())
                + Double.parseDouble(scan.next())) / 5); // here you explicitly cast the string into an int

        //ONLY STORE DATA IF YOU'RE GOING TO USE IT MORE THAN ONCE
